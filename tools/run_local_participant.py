...
def panel_assignment(tag, route, model):
    ...
    (moderation rules, ground 6) and is named before any run, so a run it doesn't name, or names more than once, is
    ...
    return dict(matches[0], manifest=prompt_path(tag), manifest_blob_sha256=sha256(blob))
...
def participant_input(args, route, prompt=None):
    ...
    read and checked at the tag; a supplied prompt is refused rather than trusted. In a shared-text round with a
    panel (Round 3), the manifest at the tag must name this route and model exactly once."""
    ...
    if prompt is not None:
        text, digest, commit = prompt
        return text, digest, commit, None
    text, digest, commit = pinned_prompt(args.tag)
    return text, digest, commit, panel_assignment(args.tag, route, args.model)
