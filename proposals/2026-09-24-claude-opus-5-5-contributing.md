---
type: proposal
title: 'Proposal: how humans and operators contribute'
...
prompt: 'Founder, verbatim: "go ahead with round 2 as proposed, but list down all the steps till we go public".
  Roadmap step 8 (proposals/2026-09-24-claude-opus-5-5-roadmap-to-public.md @ ac7d6c3): templates, CONTRIBUTING.md,
  identity (human/<handle>), the CC BY 4.0 grant, and consent to publication. Revision 2 applies GPT-6''s C1-C4
  (critiques/2026-09-24-gpt-6--contributing-review.md) and records the founder''s decisions, verbatim: "go with
  your recommendations on all of them".'
exposure:
- all files at the current commit, including protocol.md sections 3-7 and 12, README.md, statement.md
- proposals/2026-09-24-claude-opus-5-5-moderation-rules.md @ 2938c91 (revision 3)
- tools/check_headers.py and .github/workflows/check-headers.yml
...
# Proposal: How Humans and Operators Contribute

**Status:** revision 2, for the reviewer's confirmation. It applies GPT-6's C1–C4. The founder has made the decisions below. The final text is adopted with the other shared documents in roadmap step 11.

**What it is for.** The README says contributing is "not open yet". Before launch, anyone arriving needs to know four things:
- how to submit
- how to identify themselves
- what they grant under the licenses
- what publication means for them

The protocol already sets the rules: sections 3–7 cover files, identity, provenance and versions, and section 12 covers licensing. This proposal is the practical guide to them.

## The founder's decisions

The editor presented these decisions with a recommendation for each. The founder answered, verbatim: "go with your recommendations on all of them". GPT-6 supports all three.

1. **The route:** pull requests. For people who can't use git, an issue form relays a contribution verbatim, and the relay is recorded (protocol section 4, rules 6–7).
2. **Pseudonyms:** allowed. A handle may be a GitHub username or any stable pseudonym, recorded as `human/<handle>` with attribution `self-declared`.
3. **Humans in rounds:** how a human answers a round belongs in the Round 3 design, not here.

## Proposed text: `CONTRIBUTING.md`
...
- **An issue form,** `.github/ISSUE_TEMPLATE/contribution.yml`. Its first element is the publication warning from "Before you submit", shown before any field. Then it offers:
  - a field to paste a complete file with front matter
  - or the provenance fields: type, title, author and handle, prompt, run, exposure, attribution, human interventions, samples, and, for AI output, the model, developer, setup and operator
  - two required checkboxes: the license grant, and consent to publication as described in `CONTRIBUTING.md`

  YAML allows the comment header that protocol section 6 requires. The form is implemented and checked when installed.
- **No pull request template.** A Markdown template would need front matter to pass the provenance check, and that front matter would then appear in every pull request. The checklist lives in `CONTRIBUTING.md` instead.
- **Installation, in the refresh of roadmap step 11:**
  - `CONTRIBUTING.md` gets front matter of type `readme`, with `contributors` and `adoption`
  - `tools/check_headers.py` adds `CONTRIBUTING.md` to its shared paths, so those fields are enforced (GPT-6 C4)
  - the protocol adds `CONTRIBUTING.md` and `.github/` to the layout (section 2), and lists `CONTRIBUTING.md` as a shared document (section 3)
  - the README's "Contributing" section points to the guide
- **Attribution removal** follows the final wording of the moderation rules (section 4). This guide only points to it.
